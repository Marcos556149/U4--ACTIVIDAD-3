...
class Aplicacion:
    ...
    def __init__(self,lista):
        ...
        #BOTONES
        style=ttk.Style()
        self.__botonSalir=ttk.Button(marco,text="Salir",command=self.__ventana.destroy).grid(row=45,column=0,columnspan=2)
        self.__botonActualizar=ttk.Button(marco,text="Actualizar").grid(row=47,column=0,columnspan=2,rowspan=4)
        ...
        self.__fecha = ttk.Label(marco, textvariable=self.__fechaHora, width=18, foreground="#85BB65",background="#F0F0F2",font=("Arial",13)).grid(row=47,column=2,pady=18,rowspan=1)
    ...
    def actualizar(self,lista):
        for x in lista:
            nombre=x[0]
            self.__label[nombre][3].congig(text=x[1])
            self.__label[nombre][4].config(text=x[2])
